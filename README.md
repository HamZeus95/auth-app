...
> This repo adds JWT auth (access + rotating refresh tokens) on Postgres — see **[AUTH.md](AUTH.md)**.
> Run the whole app in Docker: `bun run app` → http://localhost:3001
> Local dev: `bun db:up && bun db:seed && bun dev` → http://localhost:5173
